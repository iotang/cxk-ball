class Skill {
	...
	bindKey() {
		window.addEventListener("keydown", (event) => {
			if (
				(typeof this.keyCode === "number" && event.keyCode === this.keyCode) ||
				(Array.isArray(this.keyCode) &&
					this.keyCode.indexOf(event.keyCode) >= 0)
			) {
				try {
					this.cast();
				} catch (e) {
		...
		const keyName =
			typeof this.keyCode === "number"
				? String.fromCharCode(this.keyCode)
				: this.keyCode.map((key) => String.fromCharCode(key)).join("/");

		this.delta = Math.ceil(this.cost * Math.pow(window.cacheBallSpeed, 2.9));
		console.log(
			`亦可已加载技能：[${keyName}]${this.name}，冷却 ${this.cd}，消耗 ${this.delta}`
		);
		this.lastCastTime = 0;
	}
	...
	cast() {
		...
		if (this.lastCastTime + this.cd * 1000 > nowtime) {
			let distan =
				this.cd - (this.lastCastTime + this.cd * 1000 - nowtime) / 1000.0;
		...
		console.log(
			`亦可消耗了 ${this.delta} 积分发动了技能 ${this.name}！\n${this.desc}`
		);
...
class SkillW extends Skill {
	constructor(main) {
		super(main, "轨迹干扰", "", "5 秒内让 EVA 跟着亦可", 15, 7.5, "W");
...
class SkillE extends Skill {
	constructor(main) {
		super(main, "快速迭代", "", "向当前朝向立刻移动一小段距离", 0.2, 1.25, "E");
	}
	...
	cast() {
		...
		if (move_way == 2) {
			if (ball.x < paddle.x && ball.y + 80 > paddle.y)
				paddle.x = Math.max(ball.x, paddle.x - transdis);
			else paddle.x = Math.max(-30, paddle.x - transdis);
		}
		if (move_way == 1) {
			if (ball.x > paddle.x && ball.y + 80 > paddle.y)
				paddle.x = Math.min(ball.x, paddle.x + transdis);
...
class SkillR extends Skill {
	constructor(main) {
		super(main, "超频模式", "", "砖块不会反弹 EVA，持续 10 秒", 30, 20, "R");
	...
	cast() {
		...
		const { game } = this.main;
		var isover = 0;

		this.casting = setInterval(() => {
			if (
				game.state == game.state_GAMEOVER ||
				game.state == game.state_UPDATE
			) {
				isover = 1;
				game.main.turboMode = 0;
				clearInterval(this.casting);
			}
			if (!isover) {
				game.main.turboMode = 1;
			}
		}, 125);
