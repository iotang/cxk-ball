...
class Game {
	...
	drawText(obj) {
		...
		this.context.fillText(
			globalScore + obj.allScore,
			this.canvas.width - 8 - 36 - 16,
			this.canvas.height - 10 - 72 - 36
		);
		...
		if (streakhitcnt > 0) {
			...
			this.context.font = "italic bold 24px sans-serif";
			this.context.fillText(
				((streaksum + streak) / (streakhitcnt + (streak != 0))).toFixed(4),
				this.canvas.width - 8 - 18 * 4 - 16,
				this.canvas.height / 2 - 24
			);
		}
		...
		if (streak > 0) {
			let streaksize = Math.min(
				96,
				(Math.log(streak + 1) / Math.log(51)) * (96 - 24) + 24
			);
	...
	drawSkills(obj, num) {
		...
		let distan = (obj.lastCastTime + obj.cd * 1000 - nowtime) / 1000.0;
		...
		this.context.font = "24px sans-serif";
		this.context.fillText(obj.name, num * 192 + 48, this.canvas.height - 36);
	...
	gameOver() {
		...
		this.context.fillStyle = "#000";
		this.context.fillText("亦可：我今天爆炸了！", x, y);
		this.context.font = "18px sans-serif";
		...
		if (streakhitcnt > 0) {
			...
			this.context.font = "italic bold 24px sans-serif";
			this.context.fillText(
				(streaksum / streakhitcnt).toFixed(4),
				this.canvas.width - 8 - 18 * 4 - 16,
				this.canvas.height / 2 - 24
			);
		}
	...
	goodGame() {
		...
		this.context.font = "32px sans-serif";
		this.context.fillStyle = "#000";
		this.context.fillText("亦可：迫害 EVA 完成！", x, y);
		this.context.font = "18px sans-serif";
	...
	finalGame() {
		...
		this.context.font = "32px sans-serif";
		this.context.fillStyle = "#000";
		this.context.fillText("恭喜你！你通过了所有关卡！", x, y);
		this.context.font = "18px sans-serif";
		this.context.fillText("总得分 ", x, y + 18 * 1.5);
	...
	init() {
		...
		window.addEventListener("mousedown", function (event) {
			...
			if (y <= (clientHeight * 3) / 4) {
				...
				if ((clientWidth * 0) / 4 <= x && x < (clientWidth * 1) / 4)
					skillq.cast();
				if ((clientWidth * 1) / 4 <= x && x < (clientWidth * 2) / 4)
					skillw.cast();
				if ((clientWidth * 2) / 4 <= x && x < (clientWidth * 3) / 4)
					skille.cast();
				if ((clientWidth * 3) / 4 <= x && x <= (clientWidth * 4) / 4)
					skillr.cast();
			}
		});
